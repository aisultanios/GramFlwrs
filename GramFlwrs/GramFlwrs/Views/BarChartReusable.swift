...
struct BarChart: View {
        ...
        return VStack(alignment: .center) {
            ...
            Text(update.update?.number_of_followers ?? "")
                ...
                Rectangle()
                    ...
                    .frame(height:(CGFloat(Int(update.update?.number_of_followers ?? "5")!) / maxFlwrs) * (proxy.size.height - 75))
                    .opacity(isDragging ? (currentUpdateID == update.update?.id ? 1 : 0.35) : 1)
    ...
    func getMaxFlwrs(updates: [LastUpdate]) -> CGFloat {
        ...
        let highestFollowersUpdate = updates.max { update1, update2 in
            return update1.number_of_followers ?? "5" < update2.number_of_followers ?? "5"
        }
        
        return CGFloat(Int(highestFollowersUpdate?.number_of_followers ?? "5")!)
    }
